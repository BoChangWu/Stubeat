...
function initListeners() {
  ...
  // 1旋律播放鍵:btnPlay
  document.getElementById("btnPlay").onclick = playRecording; // 要先按record 鍵
  // 旋律儲存按鍵:btnSave
...
function checkPlayer() { // state check
...
function startOrStop() {
  ...
  if (metronome.isTicking) {
    ...
    window.onkeydown = onKeydown; //不必要
...
// 播放記錄的旋律:playRecording play_step2
function playRecording() { 
  if (recorder.isPlaying) {
    ...
    updateUI("play-start");
    recorder.start(() => updateUI("play-stop")); //沒有傳() => updateUI("play-stop")也不會死 
  }
}
function playRNN() {
  recorder.musicRNN();
